...
def get_options():
  ...
  parser.add_option("--muhatd",dest="muhatd",default=None, help="muhat up")
...
if opt.mass is not None: w.var("MH").setVal(float(opt.mass))#MH
...
for cidx in range(chan.numTypes()):
  chan.setIndex(cidx)
  #c = 'pdf_bin'+chan.getLabel()
  c = chan.getLabel() 
  print "   * Category: %s"%c
  if(opt.cat!='all')&(c!=opt.cat): continue

  b_model.Print()
  bpdfs[c] = b_model.getPdf(c) 
  sbpdfs[c]  =  sb_model.getPdf(c)
...
for c in h_data:
  ...
  canv = ROOT.TCanvas("canv_%s"%c,"canv_%s"%c,800,800)
  pad1 = ROOT.TPad("pad1_%s"%c,"pad1_%s"%c,0.001,0.25,1,1)
  pad1.SetTickx()
  pad1.SetTicky()
  pad1.SetBottomMargin(0.12)
  pad1.SetRightMargin(0.03)
  pad1.SetLeftMargin(0.122)
  pad1.Draw()
  pad2 = ROOT.TPad("pad2_%s"%c,"pad2_%s"%c,0.001,0,1,0.35)
  pad2.SetTickx()
  pad2.SetTicky()
  pad2.SetTopMargin(0.005)
  pad2.SetBottomMargin(0.25)
  pad2.SetRightMargin(0.03)
  pad2.SetLeftMargin(0.122)
  pad2.Draw()
  ...
  lat0.DrawLatex(0.68,0.92,"137 fb^{-1} (13 TeV)")
  #lat0.DrawLatex(0.6,0.8,"#scale[0.75]{%s}"%Translate(c,translateCats))
  
  lat0.DrawLatex(0.143,0.83,"#scale[0.73]{H#rightarrow l^{+}l^{-}#gamma(m_{l^{+}l^{-}}>50GeV)}")
  if(opt.loadSnapshot is not None): 
    muhat, mhhat = w.var("r").getVal(), w.var("MH").getVal()#MH
    lat0.DrawLatex(0.143,0.76,"#scale[0.75]{m_{H} = %.1f GeV, #mu = %.1f^{%s}_{%s}}"%(mhhat,muhat,opt.muhatu,opt.muhatd))
  ...
  #h_axes_ratio.GetYaxis().SetNdivisions(550);
