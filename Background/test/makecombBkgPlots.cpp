...
int main(int argc, char* argv[]){
  ...
  desc.add_options()
    ...
    ("massStep,m", po::value<double>(&massStep)->default_value(1.0),						   								"Mass step for calculating bands. Use a large number like 5 for quick running")
  ...
  */
  ...
  if (doSignal){
    ...
    // if (SignalType==1) {
      ...
      TH1F *bkgHist = (TH1F*)sigFile->Get("hbkg");
      //sigHist->Add(gghHist);
      /*sigHist->Add(vbfHist);
      if (whHist) sigHist->Add(whHist);
      if (zhHist) sigHist->Add(zhHist);
      sigHist->Add(tthHist);*/
      sigHist->Scale(10.);bkgHist->Scale(10.);
      sigHist->SetLineColor(kBlue);bkgHist->SetLineColor(kMagenta);
      bkgHist->SetLineStyle(2);
      sigHist->SetLineWidth(3);bkgHist->SetLineWidth(3);
      sigHist->SetFillColor(38);
      sigHist->SetFillStyle(3001);
      sigHist->Draw("HISTsame");bkgHist->Draw("HISTsame");
      leg->AddEntry(sigHist,"10xSignal m_{H}=125GeV","LF");
      leg->AddEntry(bkgHist,"10xH#rightarrow#mu#mu m_{H}=125GeV","LF");
      //outFile->cd();
      //sigHist->Write();
  ...
  canv->Print(Form("%s/bkgplot_%s_mumu.pdf",outDir.c_str(),catname.c_str()));
  canv->Print(Form("%s/bkgplot_%s_mumu.png",outDir.c_str(),catname.c_str()));
  canv->Print(Form("%s/bkgplot_%s_mumu.C",outDir.c_str(),catname.c_str()));
  canv->SetName(Form("bkgplot_%s_mumu",catname.c_str()));
  /*outFile->cd();
  canv->Write();
  outWS->Write();
  outFile->Close();
