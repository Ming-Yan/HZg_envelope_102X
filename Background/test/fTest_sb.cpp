...
void runFit(RooAbsPdf *pdf, RooDataHist* /*RooDataSet */data, double *NLL, int *stat_t, int MaxTries){
  ...
  while (stat!=0){
	...
	//fitTest = pdf->fitTo(*data, RooFit::Save(1), RooFit::Extended(kTRUE), Range("left,right"), RooFit::Minimizer("Minuit2","minimize"),RooFit::SumW2Error(kTRUE)); //FIXME
	fitTest = pdf->fitTo(*data, RooFit::Save(1), RooFit::Extended(kTRUE), Range("left,right"), RooFit::Minimizer("Minuit2","migrad")/*,RooFit::SumW2Error(kTRUE)*/); //FIXME
      }
      else {
...
double getProbabilityFtest(double chi2, int ndof,RooAbsPdf *pdfNull, RooAbsPdf *pdfTest, RooRealVar *mass, RooDataHist* /*RooDataSet */data, std::string name){
  ...
  //= pdfNull->fitTo(*data,RooFit::Save(1),RooFit::Strategy(1)
					     // ,RooFit::Minimizer("Minuit2","minimize"),RooFit::SumW2Error(kTRUE),RooFit::PrintLevel(-1)); //FIXME
  RooFitResult *fitTestData;
  // = pdfTest->fitTo(*data,RooFit::Save(1),RooFit::Strategy(1)
					     // ,RooFit::Minimizer("Minuit2","minimize"),RooFit::SumW2Error(kTRUE),RooFit::PrintLevel(-1)); //FIXME
if (SIDEBAND) {
  fitNullData = pdfNull->fitTo(*data, RooFit::Extended(kTRUE),RooFit::Save(1),Range("left,right")/*,RooFit::Strategy(1)*/,RooFit::Minimizer("Minuit2","migrad")/*,RooFit::SumW2Error(kTRUE)*/,RooFit::PrintLevel(-1)); //FIXME
  fitTestData = pdfTest->fitTo(*data, RooFit::Extended(kTRUE),RooFit::Save(1),Range("left,right")/*,RooFit::Strategy(1)*/,RooFit::Minimizer("Minuit2","migrad")/*,RooFit::SumW2Error(kTRUE)*/,RooFit::PrintLevel(-1)); //FIXME
  }
  else {
  ...
  for (int itoy = 0 ; itoy < ntoys ; itoy++){
    ...
    while (stat_n!=0){
      ...
      // = pdfNull->fitTo(*binnedtoy,RooFit::Save(1),RooFit::Strategy(1),RooFit::SumW2Error(kTRUE) //FIXME
				//	     ,RooFit::Minimizer("Minuit2","minimize"),RooFit::Minos(0),RooFit::Hesse(0),RooFit::PrintLevel(-1));
      //,RooFit::Optimize(0));
      if (SIDEBAND) {
	fitNull = pdfNull->fitTo(*binnedtoy, RooFit::Extended(kTRUE),Range("left,right"),RooFit::Save(1)/*,RooFit::Strategy(1),RooFit::SumW2Error(kTRUE)*/,RooFit::Minimizer("Minuit2","migrad")/*,RooFit::Minos(0),RooFit::Hesse(0)*/,RooFit::PrintLevel(-1));
      }
      else {
    ...
    while (stat_t!=0){
      if (ntries>=MaxTries) break;
      RooFitResult *fitTest;
       // = pdfTest->fitTo(*binnedtoy,RooFit::Save(1),RooFit::Strategy(1),RooFit::SumW2Error(kTRUE) //FIXME
					     // ,RooFit::Minimizer("Minuit2","minimize"),RooFit::Minos(0),RooFit::Hesse(0),RooFit::PrintLevel(-1));
       if (SIDEBAND) {
          fitTest = pdfTest->fitTo(*binnedtoy, RooFit::Extended(kTRUE),Range("left,right"),RooFit::Save(1),/*RooFit::Strategy(1),RooFit::SumW2Error(kTRUE),*/RooFit::Minimizer("Minuit2","migrad")/*,RooFit::Minos(0),RooFit::Hesse(0)*/,RooFit::PrintLevel(-1));
      }
      else {
...
double getGoodnessOfFit(RooRealVar *mass, RooAbsPdf *mpdf, RooDataHist* /*RooDataSet*/data, std::string name){
  ...
  //pdf->plotOn(plot_chi2, RooFit::LineColor(2),RooFit::NormRange("full"), RooFit::Range("left"), RooFit::Normalization(1,RooAbsReal::RelativeExpected));
  //pdf->plotOn(plot_chi2, RooFit::LineColor(2),RooFit::NormRange("full"), RooFit::Range("right"), RooFit::Normalization(1,RooAbsReal::RelativeExpected));
  // pdf->plotOn(plot_chi2,Name("pdf"),RooFit::Range("left,right"), RooFit::NormRange("left,right"));
  ...
  if ((double)data->sumEntries()/nBinsForMass < 5 ){
	...
	pdf->fitTo(*binnedtoy,Range("left,right"),RooFit::Minimizer("Minuit2","migrad"),RooFit::Minos(0),RooFit::Hesse(0),RooFit::PrintLevel(-1)/*,RooFit::Strategy(0),RooFit::SumW2Error(kTRUE)*/); //FIXME
      ...
      pdf->plotOn(plot_t, RooFit::Range("left,right"), RooFit::NormRange("full"), RooFit::Normalization(1,RooAbsReal::RelativeExpected));
      //pdf->plotOn(plot_t, RooFit::NormRange("full"), RooFit::Range("left"), RooFit::Normalization(1,RooAbsReal::RelativeExpected));
      //pdf->plotOn(plot_t,RooFit::NormRange("full"), RooFit::Range("right"), RooFit::Normalization(1,RooAbsReal::RelativeExpected));
...
void plot(RooRealVar *mass, RooAbsPdf *pdf, RooDataHist /*RooDataSet */*data, string name,vector<string> flashggCats_, int status, double *prob){
  ...
  *prob = getGoodnessOfFit(mass,pdf,data,name);
  ...
  if (SIDEBAND) {
    ...
    pdf->plotOn(plot, RooFit::NormRange("full"), RooFit::Range("left,right"), RooFit::Normalization(1,RooAbsReal::RelativeExpected));
    //pdf->plotOn(plot, RooFit::NormRange("full"), RooFit::Range("left"), RooFit::Normalization(1,RooAbsReal::RelativeExpected));
    //pdf->plotOn(plot, RooFit::NormRange("full"), RooFit::Range("right"), RooFit::Normalization(1,RooAbsReal::RelativeExpected));
...
void plot(RooRealVar *mass, RooMultiPdf *pdfs, RooCategory *catIndex,RooRealVar *bkgnorm, RooDataHist/*RooDataSet*/ *data, string name, string type,vector<string> flashggCats_, int cat, int bestFitPdf=-1){
  ...
  for (int icat=0;icat<catIndex->numTypes();icat++){
    ...
    if (SIDEBAND) {
      pdfs->getCurrentPdf()->fitTo(*data,RooFit::Extended(kTRUE),Range("left,right"),RooFit::Minimizer("Minuit2","migrad"),RooFit::SumW2Error(kTRUE));  //FIXME
      // pdfs->getCurrentPdf()->plotOn(plot, RooFit::LineColor(col),RooFit::NormRange("full"), RooFit::Range("left"), RooFit::Normalization(1,RooAbsReal::RelativeExpected));
      // pdfs->getCurrentPdf()->plotOn(plot, RooFit::LineColor(col),RooFit::NormRange("full"), RooFit::Range("right"), RooFit::Normalization(1,RooAbsReal::RelativeExpected));
      pdfs->getCurrentPdf()->plotOn(plot,LineColor(col),LineStyle(style),RooFit::Range("left,right"),RooFit::NormRange("left,right"));
      //pdfs->getCurrentPdf()->plotOn(plot,RooFit::NormRange("full"),RooFit::Range("left"),RooFit::Normalization(1,RooAbsReal::RelativeExpected),RooFit::LineColor(col));
      //pdfs->getCurrentPdf()->plotOn(plot,RooFit::NormRange("full"),RooFit::Range("right"),RooFit::Normalization(1,RooAbsReal::RelativeExpected),RooFit::LineColor(col));
    }
    else {
    ...
    string pname = pdfs->getCurrentPdf()->GetName();
    string p1= "env_pdf_ele_mu";
    size_t i = pname.find(p1);
    if(i!=std::string::npos)pname.erase(i,p1.length());
    p1 = "2020";
    if(i!=std::string::npos)pname.erase(i,p1.length());
    p1 = "_13TeV_";    if(i!=std::string::npos)pname.erase(i,p1.length());
    leg->AddEntry(pdfLeg, Form("%s%s",pname.c_str(),ext.c_str()),"L");    
  }
...
void plot(RooRealVar *mass, map<string,RooAbsPdf*> pdfs, RooDataHist/*RooDataSet*/ *data, string name, string type, vector<string> flashggCats_, int cat, int bestFitPdf=-1){
 ...
 int color[7] = {kBlue,kRed,kMagenta,kGreen+1,kOrange+7,kAzure+10,kBlack};
  ...
  for (map<string,RooAbsPdf*>::iterator it=pdfs.begin(); it!=pdfs.end(); it++){
    ...
    //it->second->plotOn(plot, RooFit::NormRange("full"), RooFit::Range("left"), RooFit::Normalization(1,RooAbsReal::RelativeExpected), RooFit::LineColor(col));
    it->second->plotOn(plot,RooFit::NormRange("full"), RooFit::Range("left"), RooFit::Normalization(1,RooAbsReal::RelativeExpected), RooFit::LineColor(col));
    it->second->plotOn(plot,RooFit::NormRange("full"), RooFit::Range("right"), RooFit::Normalization(1,RooAbsReal::RelativeExpected), RooFit::LineColor(col));
    //it->second->plotOn(plot, RooFit::NormRange("full"), RooFit::Range("right"), RooFit::Normalization(1,RooAbsReal::RelativeExpected), RooFit::LineColor(col));
    
      //it->second->plotOn(plot,LineColor(col),LineStyle(style),RooFit::NormRange("left,right"));//,RooFit::NormRange("fitdata_1,fitdata_2"));

    //it->second->plotOn(plot,LineColor(col),LineStyle(1), RooFit::NormRange("left,right"),RooFit::NormRange("left,right"));
    
    TObject *pdfLeg = plot->getObject(int(plot->numItems()-1));

    std::string ext = "";
    if (bestFitPdf==i) ext=" (Best Fit Pdf) ";
    //leg->AddEntry(pdfLeg,Form("%s",ext.c_str()),"L");
...
int getBestFitFunction(RooMultiPdf *bkg, RooDataHist/*RooDataSet */*data, RooCategory *cat, bool silent=false){
...
int main(int argc, char* argv[]){
  ...
  mass->setRange(115, 170); // JB for fit study (FULL RANGE)
  //mass->setRange("left", rangeMin, 120.);
  //mass->setRange("right", 130., rangeMax);
  mass->setRange("full", 115, 170.);
  mass->setRange("left", 115, 120.);
  mass->setRange("right", 130., 170);
  ...
  for (int cat=startingCategory; cat<ncats; cat++){
    ...
    if ( isFlashgg_){
      ...
      data1 = (RooDataSet*)dataFull;
    } else {
      thisdataBinned_name= Form("roohist_data_mass_cat%d",cat);
      data1 = (RooDataSet*)dataFull;
    }
    RooDataHist thisdataBinned(Form("data_%s_%s_%d",type.c_str(),flashggCats_[cat].c_str(),period),"data",*mass,*dataFull);
    // RooDataSet *data = (RooDataSet*)&thisdataBinned;
    RooDataHist *data = &thisdataBinned;
	...
	if (!bkgPdf){
	  ...
	  if (prev_pdf!=NULL){
	    prob = getProbabilityFtest(chi2,order-prev_order,prev_pdf,bkgPdf,mass,data
				       ,Form("%s/Ftest_from_%s_%d_%s_%s_%d_hist.pdf",outDir.c_str(),funcType->c_str(),order,type.c_str(),flashggCats_[cat].c_str(),period));
	  ...
	  if (!saveMultiPdf) plot(mass,bkgPdf,data,Form("%s/%s%d_%s_%s_%d_sb.pdf",outDir.c_str(),funcType->c_str(),order,type.c_str(),flashggCats_[cat].c_str(),period),flashggCats_,fitStatus,&gofProb);
	...
	while (prob<upperEnvThreshold){
	  ...
	  if (!bkgPdf ){
	    ...
	    //bkgPdf->fitTo(*data,Save(true),RooFit::Minimizer("Minuit2","minimizen"));
	    //thisNll = fitRes->minNll();
	    if (fitStatus!=0) std::cout << "[WARNING] Warning -- Fit status for " << bkgPdf->GetName() << " at " << fitStatus <<std::endl;
	    ...
	    double gofProb =0; 
	    plot(mass,bkgPdf,data,Form("%s/%s%d_%s_%s_%d_sb.pdf",outDir.c_str(),funcType->c_str(),order,type.c_str(),flashggCats_[cat].c_str(),period),flashggCats_,fitStatus,&gofProb);
    ...
    plot(mass,pdfs,data,Form("%s/truths_%s_%s_%d_sb",outDir.c_str(),type.c_str(),flashggCats_[cat].c_str(),period),type.c_str(),flashggCats_,cat);
	...
	catindexname = Form("pdfindex_%s_%s_%d_%s",type.c_str(),flashggCats_[cat].c_str(),period,ext.c_str());
      ...
      plot(mass,pdf,&catIndex,&nBackground,data,Form("%s/multipdf_%s_sb",outDir.c_str(),catname.c_str()),type,flashggCats_,cat,bestFitPdfIndex);
      outputws->Print();
    }
