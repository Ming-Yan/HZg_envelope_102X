...
int mzg_low =100;//mod-zg
int mzg_high =170;//mod-zg
int nBinsForMass = 4*(mzg_high-mzg_low);

TRandom3 *RandomGen = new TRandom3();

RooAbsPdf* getPdf(PdfModelBuilder &pdfsModel, string type, int order, const char* ext,string cats){

  int cat = 0;
  if (boost::algorithm::contains(cats,"501"))cat =501;
  else if(boost::algorithm::contains(cats,"502"))cat =502;
  else if(boost::algorithm::contains(cats,"503")) cat = 503;
  else if(boost::algorithm::contains(cats,"6789")) cat = 6789;
  else if(boost::algorithm::contains(cats,"1")) cat = 1;
  else if(boost::algorithm::contains(cats,"2")) cat = 2;
  else if(boost::algorithm::contains(cats,"3")) cat = 3;
  else if(boost::algorithm::contains(cats,"4")) cat = 4;
  if (type=="Bernstein") return pdfsModel.getBernsteinStepxGau(Form("%s_bern%d",ext,order),order);
  ...
  else if (type=="Exponential") return pdfsModel.getExponentialStepxGau(Form("%s_exp%d",ext,order),order,cat);
  // return pdfsModel.getExponentialSingle(Form("%s_exp%d",ext,order),order); 
  else if (type=="PowerLaw") return pdfsModel.getPowerLawStepxGau(Form("%s_exp%d",ext,order),order,cat);
  // return pdfsModel.getPowerLawSingle(Form("%s_pow%d",ext,order),order); 
  else if (type=="Laurent") return pdfsModel.getLaurentStepxGau(Form("%s_lau  %d",ext,order),order,cat);
...
double getProbabilityFtest(double chi2, int ndof,RooAbsPdf *pdfNull, RooAbsPdf *pdfTest, RooRealVar *mass, RooDataSet *data, std::string name){
  ...
  can->SetLogy();
  TH1F toyhist(Form("toys_fTest_%s_turn.pdf",pdfNull->GetName()),";Chi2;",60,-2,10);
  TH1I toyhistStatN(Form("Status_%s_turn.pdf",pdfNull->GetName()),";FitStatus;",8,-4,4);
  TH1I toyhistStatT(Form("Status_%s_turn.pdf",pdfTest->GetName()),";FitStatus;",8,-4,4);
  ...
  leg->Draw();
  stas->SaveAs(Form("%s_fitstatus_turn_turn.pdf",name.c_str()));
  //reassign params
  params_null->assignValueOnly(preParams_null);
  params_test->assignValueOnly(preParams_test);
...
double getGoodnessOfFit(RooRealVar *mass, RooAbsPdf *mpdf, RooDataSet *data, std::string name){
  ...
  if ((double)data->sumEntries()/nBinsForMass < 5 ){
    ...
    TH1F toyhist(Form("gofTest_%s_turn.pdf",pdf->GetName()),";Chi2;",50,medianChi2-5*rms,medianChi2+5*rms);
...
void plot(RooRealVar *mass, RooMultiPdf *pdfs, RooCategory *catIndex, RooDataSet *data, string name, string type,vector<string> flashggCats_, int cat, int bestFitPdf=-1){
  ...
  // enf extra bit for ratio plot///
  canv->SaveAs(Form("%s_turn.pdf",name.c_str()));
  canv->SaveAs(Form("%s_turn.png",name.c_str()));
...
int main(int argc, char* argv[]){
  ...
  RooWorkspace *inWS;
  if(isFlashgg_){
    if (isData_)inWS = (RooWorkspace*)inFile->Get("multipdf");//mod-zg
  ...
  for (int cat=startingCategory; cat<ncats; cat++){
    ...
    for (vector<string>::iterator funcType=functionClasses.begin(); 
	 funcType!=functionClasses.end(); funcType++){
      ...
      int counter =0;
      //	while (prob<0.05){
      while (prob<0.01 && order < 7){ //FIXME
	RooAbsPdf *bkgPdf = getPdf(pdfsModel,*funcType,order,Form("ftest_pdf_%s_%s_%d_%s",type.c_str(),flashggCats_[cat].c_str(),period,ext.c_str()),catname);
	if (!bkgPdf){
	  ...
	  if (prev_pdf!=NULL){
	    prob = getProbabilityFtest(chi2,order-prev_order,prev_pdf,bkgPdf,mass,data
				       ,Form("%s/Ftest_from_%s_%d_%s_%s_%d_turn.pdf",outDir.c_str(),funcType->c_str(),order,type.c_str(),flashggCats_[cat].c_str(),period));
	  ...
	  if (!saveMultiPdf) plot(mass,bkgPdf,data,Form("%s/%s%d_%s_%s_%d_turn.pdf",outDir.c_str(),funcType->c_str(),order,type.c_str(),flashggCats_[cat].c_str(),period),flashggCats_,fitStatus,&gofProb);
	...
	while (prob<upperEnvThreshold){
	  RooAbsPdf *bkgPdf = getPdf(pdfsModel,*funcType,order,Form("env_pdf_%s_%s_%d_%s",type.c_str(),flashggCats_[cat].c_str(),period,ext.c_str()),catname);
	  if (!bkgPdf ){
	    // assume this order is not allowed
	    ...
	    double gofProb =0; 
	    plot(mass,bkgPdf,data,Form("%s/%s%d_%s_%s_%d_turn.pdf",outDir.c_str(),funcType->c_str(),order,type.c_str(),flashggCats_[cat].c_str(),period),flashggCats_,fitStatus,&gofProb);
