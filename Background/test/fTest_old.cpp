...
int main(int argc, char* argv[]){
  ...
  desc.add_options()
    ...
    ("is2016",                                                                                  "Run 2016 config")
    ("is2017",                                                                                  "Run 2017 config")
  ...
  for (int cat=startingCategory; cat<ncats; cat++){
    ...
    string catname;
    if (isFlashgg_)catname = Form("%s_%s",type.c_str(),flashggCats_[cat].c_str());
    ...
    if ( isFlashgg_){
      thisdataBinned_name =Form("roohist_data_mass_%s_%s",type.c_str(),flashggCats_[cat].c_str());
	...
	RooAbsPdf *bkgPdf = getPdf(pdfsModel,*funcType,order,Form("ftest_pdf_%s_%s_%s",type.c_str(),flashggCats_[cat].c_str(),ext.c_str()));
	if (!bkgPdf){
	  ...
	  if (prev_pdf!=NULL){
	    prob = getProbabilityFtest(chi2,order-prev_order,prev_pdf,bkgPdf,mass,data
				       ,Form("%s/Ftest_from_%s_%d_%s_%s.pdf",outDir.c_str(),funcType->c_str(),order,type.c_str(),flashggCats_[cat].c_str()));
	  ...
	  if (!saveMultiPdf) plot(mass,bkgPdf,data,Form("%s/%s%d_%s_%s.pdf",outDir.c_str(),funcType->c_str(),order,type.c_str(),flashggCats_[cat].c_str()),flashggCats_,fitStatus,&gofProb);
	...
	while (prob<upperEnvThreshold){
	  RooAbsPdf *bkgPdf = getPdf(pdfsModel,*funcType,order,Form("env_pdf_%s_%s_%s",type.c_str(),flashggCats_[cat].c_str(),ext.c_str()));
	  if (!bkgPdf ){
	    // assume this order is not allowed
	    ...
	    double gofProb =0; 
	    plot(mass,bkgPdf,data,Form("%s/%s%d_%s_%s.pdf",outDir.c_str(),funcType->c_str(),order,type.c_str(),flashggCats_[cat].c_str()),flashggCats_,fitStatus,&gofProb);
    ...
    plot(mass,pdfs,data,Form("%s/truths_%s_%s",outDir.c_str(),type.c_str(),flashggCats_[cat].c_str()),type.c_str(),flashggCats_,cat);

    if (saveMultiPdf){
      // Put selectedModels into a MultiPdf
      string catindexname;
      if (isFlashgg_){
	catindexname = Form("pdfindex_%s_%s_%s",type.c_str(),flashggCats_[cat].c_str(),ext.c_str());
      } else {
	catindexname = Form("pdfindex_%s_%s_%s",type.c_str(),flashggCats_[cat].c_str(),ext.c_str());
      }
      RooCategory catIndex(catindexname.c_str(),"c");
      RooMultiPdf *pdf = new RooMultiPdf(Form("CMS_hzg_%s_%s_%s_bkgshape",type.c_str(),flashggCats_[cat].c_str(),ext.c_str()),"all pdfs",catIndex,storedPdfs);
      RooRealVar nBackground(Form("CMS_hzg_%s_%s_%s_bkgshape_norm",type.c_str(),flashggCats_[cat].c_str(),ext.c_str()),"nbkg",data->sumEntries(),0,10E8);
