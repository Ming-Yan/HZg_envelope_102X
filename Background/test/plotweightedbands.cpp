...
void OptionParser(int argc, char *argv[]){
	...
	desc1.add_options()
		...
		("lumi", po::value<float>(&intLumi_)->default_value(137), 			"Output file name")
...
int main(int argc, char *argv[]) {
  ...
  for (int i=0; i<chan->numTypes(); ++i) {
    ...
    TH1D *hsbtmp = (TH1D*)sbcatpdf->createHistogram("hsbtmp",*mass,Binning(2200));
    TH1D *hbtmp = (TH1D*) bcatpdf->createHistogram("hbtmp",*mass,Binning(2200));
  ...
  TH1D *hsig = new TH1D("hsig","",2200,115.,170.);
  TH1D *hwsig = new TH1D("hwsig","",2200,115.,170.);
  
  TH1D *hbkg = new TH1D("hbkg","",2200,115.,170.);
  TH1D *hwbkg = new TH1D("hwbkg","",2200,115.,170.);

  TH1D *hbkgplot = new TH1D("hbkgplot","",nbins,lowedge,highedge);
  TH1D *hwbkgplot = new TH1D("hwbkgplot","",nbins,lowedge,highedge);
  
  TH1D *hsigbkg = new TH1D("hsigbkg","",2200,115.,170.);
  TH1D *hwsigbkg = new TH1D("hwsigbkg","",2200,115.,170.);   
  ...
  for (int i=0; i<chan->numTypes(); ++i) {
    ...
    TH1D *hsigplot_cat = new TH1D(TString::Format("hsigplot_cat%s",catnames[i].Data()),"",2200,115.,170.);
...
//  } 
  ...
  hwsigbkg->SetLineWidth(2);
  hbkg->SetLineWidth(2);
  hwbkg->SetLineWidth(2);
  hsig->SetLineWidth(2);
  hwsig->SetLineWidth(2);
  
  hsigbkgplotfine->SetLineWidth(2);
  hwsigbkgplotfine->SetLineWidth(2);
  hbkgplotfine->SetLineWidth(2);
  hwbkgplotfine->SetLineWidth(2);
  ...
  for (int isig=0; isig<hsigplotsfine.size(); ++isig) {
    ...
    hsigplotsfine[isig]->SetLineWidth(2);
  }  
...
//     pad2->SetBorderMode(0);
    ...
    //RooPlot *catplot = mass->frame(115.,170.,80);
    ...
    std::cout << "[INFO] draw " << chan->getLabel() << " s and b pdfs" << std::endl; 
      catbpdf->plotOn(catplot,Normalization(catbpdf->expectedEvents(catdatam->get()),RooAbsPdf::NumEvent),LineColor(kRed),LineStyle(kDashed),LineWidth(2));  
      sbcatpdf->plotOn(catplot,Normalization(sbcatpdf->expectedEvents(catdatam->get()),RooAbsPdf::NumEvent),LineColor(kRed),LineWidth(2));      
    }
    else if (i==ncats) {
    std::cout << "[INFO] draw " << "unweighted " << " s and b pdfs" << std::endl; 
      hbkgplotfine->Draw("histSAME");
      hbkgplot->Draw("histSAME");
      hsigbkgplotfine->Draw("histSAME");
    }
    else if (i==(ncats+1)) {
    std::cout << "[INFO] draw " << "weighted " << " s and b pdfs" << std::endl; 
      hwbkgplotfine->Draw("histSAME");
      hwsigbkgplotfine->Draw("histSAME");      
    }
    
    catdatam->plotOn(catplot,XErrorSize(0),MarkerSize(defmarkersize),MarkerStyle(defmarkerstyle),LineWidth(2));
    ...
    TLegend *leg2 = new TLegend(0.529,0.5+offset,0.860,0.749+offset);
    ...
    if (quoteMu_){
    lat2->DrawLatex(0.159,0.8,Form("#hat{m}_{H}=%.1f GeV, #hat{#mu}=%.1f",MH->getVal(),r->getVal()));
    } else {
    ...
    mytext->DrawLatex(0.129+0.03,0.85,"H#rightarrowll#gamma");
